...
import { agentAApi, arenaApi, type AuditEntry, type TurnData } from '@/lib/api'
...
export default function AgentAPage() {
  ...
  const defaultPrivateInfo = `I'm Alex, and I want to contribute to our family trust fund, but I have some constraints:

- My current monthly income is $8,500 after taxes
- I have student loans requiring $800/month minimum payment
- I'd like to keep at least $2,000/month for personal expenses and savings
- My absolute maximum contribution would be $4,000/month
- I'd prefer to start around $2,500/month to be safe
- My walk-away point (BATNA): If we can't agree on something at or below $3,500/month, I'd rather delay setting up the trust until next year when my loans are paid off

I don't want Jordan to know my exact constraints because they might push me to my maximum.`
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        // Get audit trail
        const auditData = await agentAApi.getAudit(sessionId)
  ...
  return (
    <div className="page-container">
      ...
      <div className="page-header">
        <div>
          <h1 className="page-title" style={{ background: 'linear-gradient(135deg, #3b82f6, #60a5fa)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
            Partner A - Alex
          </h1>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <span className="agent-badge agent-badge-a">Agent A</span>
        </div>
      </div>
      ...
      {!joined && (
        <div className="card">
          <h2 className="card-title">Join Negotiation as Partner A (Alex)</h2>
          <p style={{ color: 'var(--text-muted)', marginBottom: '1.5rem' }}>
            Enter the session ID from the Arena and your private financial information.
            This information stays on your server and is never shared with Partner B.
          </p>
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            ...
            <textarea
              className="textarea"
              placeholder="Enter your private constraints, budget, walk-away point..."
              value={privateInfo}
          ...
          <div style={{
            ...
            borderRadius: '8px',
            marginBottom: '1.5rem',
            borderLeft: '3px solid var(--agent-a)'
          }}>
            <strong style={{ color: 'var(--agent-a)' }}>Privacy Guarantee:</strong>
            <p style={{ color: 'var(--text-secondary)', marginTop: '0.25rem', fontSize: '0.875rem' }}>
              Your private information is processed only on Agent A's server. It physically cannot
              reach Partner B because it never leaves this backend service.
            </p>
          </div>
      ...
      {joined && (
        <>
          ...
          <div className="session-info">
            ...
            <span className="agent-badge agent-badge-a">You are Alex (Partner A)</span>
          </div>
          ...
          {auditTrail.length === 0 && !isComplete && (
            <div className="card" style={{ textAlign: 'center', padding: '3rem' }}>
              ...
              <p style={{ color: 'var(--text-muted)', marginTop: '0.5rem' }}>
                ...
                once Partner B joins and the Arena starts the session.
              </p>
            </div>
          )}
                  ...
                  <div key={idx} className="turn-card">
                    ...
                    >
                      <div className="turn-number">
                        ...
                        <span className="strategy-badge">{entry.strategy}</span>
                      </div>
                        ...
                        {publicTurn && (
                          <div className="message-block message-block-b">
                            <div className="message-sender" style={{ color: 'var(--agent-b)' }}>
                              Jordan's Response (Partner B)
                            </div>
                            <div className="message-text">{publicTurn.agent_b_message}</div>
                          </div>
                        )}

                        {/* BATNA Analysis */}
                        <div className="batna-section">
                          <div className="batna-header">BATNA Analysis (Walk-Away Point)</div>
                        ...
                        <div className="thinking-section">
                          ...
                          {entry.thinking_steps.map((step, stepIdx) => (
                            <div key={stepIdx} className="thinking-step">
                              <div className="thinking-step-number">{stepIdx + 1}</div>
